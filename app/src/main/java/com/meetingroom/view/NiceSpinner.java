...
public class NiceSpinner extends TextView {
    ...
    public GroupTypeDialog dialog;
    public void setArrowHide(boolean arrowHide) {
        isArrowHide = arrowHide;
    }
...
//        setBackgroundResource(R.drawable.selector_grey);
        textColor = typedArray.getColor(R.styleable.NiceSpinner_textTint,-1);
        textItemColor = typedArray.getColor(R.styleable.NiceSpinner_textItemTint, -1);
        ...
        if (!isArrowHide) {
            ...
            arrowTintColor = typedArray.getColor(R.styleable.NiceSpinner_arrowTint, context.getResources().getColor(R.color.white_arrow));
            if (basicDrawable != null) {
                drawable = DrawableCompat.wrap(basicDrawable);
                if (arrowTintColor != -1) {
                    DrawableCompat.setTint(drawable, arrowTintColor);
                }else{
    ...
    public void setSelectedIndex(int position) {
        if(!showInCenter){
            ...
            selectedIndex = position;
            dialog.adapter.index = selectedIndex;
            dialog.adapter.notifyDataSetChanged();
            setText(dialog.grouptype.get(selectedIndex));
            if (onItemClickListener != null) {
                onItemClickListener.onItemClick(null, null, selectedIndex, -1);
            }

            if (onItemSelectedListener != null) {
                onItemSelectedListener.onItemSelected(null, null, selectedIndex, -1);
            }
        }
    }
    ...
    public <T> void attachDataSource(@NonNull List<T> dataset) {
        adapter = new NiceSpinnerAdapter<>(getContext(), dataset, textItemColor, backgroundSelector);
    ...
    public void setShowInCenter(boolean showInCenter, List<String> grouptype, final int selectedIndex) {
        if(showInCenter){
            this.showInCenter = showInCenter;
            this.selectedIndex = selectedIndex;
            dialog = new GroupTypeDialog(mContext);
            dialog.init(grouptype, selectedIndex, new GroupTypeDialog.GroupTypeDialogCallback() {
                @Override
                public void callback(int index) {
                    NiceSpinner.this.selectedIndex = index;
                    setSelectedIndex(NiceSpinner.this.selectedIndex);
