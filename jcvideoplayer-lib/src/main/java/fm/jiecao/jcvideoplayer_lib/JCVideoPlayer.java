...
public abstract class JCVideoPlayer extends FrameLayout implements JCMediaPlayerListener, View.OnClickListener, SeekBar.OnSeekBarChangeListener, View.OnTouchListener, TextureView.SurfaceTextureListener {
    ...
    public static final int FULLSCREEN_ID = R.id.fullScreen;
    public static final int TINY_ID       = R.id.tiny;
...
//    public static long    CLICK_QUIT_FULLSCREEN_TIME = 0;
    ...
    public void onClick(View v) {
        int i = v.getId();
        if (i == R.id.start) {
            JCLogUtil.i(TAG, "onClick start [" + this.hashCode() + "] ");
            ...
            if (currentState == CURRENT_STATE_NORMAL || currentState == CURRENT_STATE_ERROR) {
                ...
                JCLogUtil.d(TAG, "pauseVideo [" + this.hashCode() + "] ");
            ...
            JCLogUtil.i(TAG, "onClick fullscreen [" + this.hashCode() + "] ");
            if (currentState == CURRENT_STATE_AUTO_COMPLETE) return;
            if (currentScreen == SCREEN_WINDOW_FULLSCREEN) {
                //quit fullscreen
                backPress();
            } else {
                JCLogUtil.d(TAG, "toFullscreenActivity [" + this.hashCode() + "] ");
                onEvent(JCBuriedPoint.ON_ENTER_FULLSCREEN);
                startWindowFullscreen();
            }
        } else if (i == R.id.surface_container && currentState == CURRENT_STATE_ERROR) {
            JCLogUtil.i(TAG, "onClick surfaceContainer State=Error [" + this.hashCode() + "] ");
            prepareVideo();
        }
    }

    public void prepareVideo() {
        JCLogUtil.d(TAG, "prepareVideo [" + this.hashCode() + "] ");
        ...
        if (id == R.id.surface_container) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    JCLogUtil.i(TAG, "onTouch surfaceContainer actionDown [" + this.hashCode() + "] ");
                ...
                case MotionEvent.ACTION_MOVE:
                    JCLogUtil.i(TAG, "onTouch surfaceContainer actionMove [" + this.hashCode() + "] ");
                ...
                case MotionEvent.ACTION_UP:
                    JCLogUtil.i(TAG, "onTouch surfaceContainer actionUp [" + this.hashCode() + "] ");
    ...
    public void addTextureView() {
        JCLogUtil.d(TAG, "addTextureView [" + this.hashCode() + "] ");
    ...
    public void onPrepared() {
        JCLogUtil.i(TAG, "onPrepared " + " [" + this.hashCode() + "] ");
...
//                .findViewById(Window.ID_ANDROID_CONTENT);
    ...
    public void onAutoCompletion() {
        JCLogUtil.i(TAG, "onAutoCompletion " + " [" + this.hashCode() + "] ");
    ...
    public void onCompletion() {
        JCLogUtil.i(TAG, "onCompletion " + " [" + this.hashCode() + "] ");
        setUiWitStateAndScreen(CURRENT_STATE_NORMAL);
        if (textureViewContainer.getChildCount() > 0) {
            textureViewContainer.removeAllViews();
        }
    ...
    public boolean backToOtherListener() {//这里这个名字这么写并不对,这是在回退的时候gotoother,如果直接gotoother就不叫这个名字
        JCLogUtil.i(TAG, "backToOtherListener " + " [" + this.hashCode() + "] ");
...
//            CLICK_QUIT_FULLSCREEN_TIME = System.currentTimeMillis();
    ...
    public void goBackThisListener() {
        JCLogUtil.i(TAG, "goBackThisListener " + " [" + this.hashCode() + "] ");
    ...
    public void onBufferingUpdate(int percent) {
        if (currentState != CURRENT_STATE_NORMAL && currentState != CURRENT_STATE_PREPARING) {
            JCLogUtil.v(TAG, "onBufferingUpdate " + percent + " [" + this.hashCode() + "] ");
    ...
    public void onError(int what, int extra) {
        JCLogUtil.e(TAG, "onError " + what + " - " + extra + " [" + this.hashCode() + "] ");
        if (what != 38 && what != -38) {
    ...
    public void onInfo(int what, int extra) {
        JCLogUtil.d(TAG, "onInfo what - " + what + " extra - " + extra);
        if (what == IMediaPlayer.MEDIA_INFO_BUFFERING_START) {
            ...
            JCLogUtil.d(TAG, "MEDIA_INFO_BUFFERING_START");
            ...
            JCLogUtil.d(TAG, "MEDIA_INFO_BUFFERING_END");
        } else if (what == IMediaPlayer.MEDIA_INFO_VIDEO_ROTATION_CHANGED) {
            JCMediaManager.instance().videoRotation = extra;
            JCMediaManager.textureView.setRotation(extra);
            JCLogUtil.d(TAG, "MEDIA_INFO_VIDEO_ROTATION_CHANGED");
        }
    }

    @Override
    public void onVideoSizeChanged() {
        JCLogUtil.i(TAG, "onVideoSizeChanged " + " [" + this.hashCode() + "] ");
        JCMediaManager.textureView.setVideoSize(JCMediaManager.instance().getVideoSize());
    }

    @Override
    public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
        JCLogUtil.i(TAG, "onSurfaceTextureAvailable [" + this.hashCode() + "] ");
    ...
    public void onStartTrackingTouch(SeekBar seekBar) {
        JCLogUtil.i(TAG, "bottomProgress onStartTrackingTouch [" + this.hashCode() + "] ");
        cancelProgressTimer();
    ...
    public void onStopTrackingTouch(SeekBar seekBar) {
        JCLogUtil.i(TAG, "bottomProgress onStopTrackingTouch [" + this.hashCode() + "] ");
        ...
        JCLogUtil.i(TAG, "seekTo " + time + " [" + this.hashCode() + "] ");
    }

    public static boolean backPress() {
        JCLogUtil.i(TAG, "backPress");
        if (JCVideoPlayerManager.getFirst() != null) {
    ...
    public void startWindowFullscreen() {
        JCLogUtil.i(TAG, "startWindowFullscreen " + " [" + this.hashCode() + "] ");
...
//            jcVideoPlayer.setAnimation(ra);
    ...
    public void startWindowTiny() {
        JCLogUtil.i(TAG, "startWindowTiny " + " [" + this.hashCode() + "] ");
    ...
    public class ProgressTimerTask extends TimerTask {
        ...
        public void run() {
            if (currentState == CURRENT_STATE_PLAYING || currentState == CURRENT_STATE_PAUSE || currentState == CURRENT_STATE_PLAYING_BUFFERING_START) {
                ...
                int duration = getDuration();
                JCLogUtil.v(TAG, "onProgressUpdate " + position + "/" + duration + " [" + this.hashCode() + "] ");
        ...
        public void onAudioFocusChange(int focusChange) {
            switch (focusChange) {
                ...
                case AudioManager.AUDIOFOCUS_LOSS:
                    releaseAllVideos();
                    JCLogUtil.d(TAG, "AUDIOFOCUS_LOSS [" + this.hashCode() + "]");
                    break;
                case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT:
                    if (JCMediaManager.instance().mediaPlayer.isPlaying()) {
                        JCMediaManager.instance().mediaPlayer.pause();
                    }
                    JCLogUtil.d(TAG, "AUDIOFOCUS_LOSS_TRANSIENT [" + this.hashCode() + "]");
                    break;
...
//                (System.currentTimeMillis() - CLICK_QUIT_FULLSCREEN_TIME) > FULL_SCREEN_NORMAL_DELAY) {
        JCLogUtil.d(TAG, "release [" + this.hashCode() + "]");
        releaseAllVideos();
//        }
    }
    ...
    public static void releaseAllVideos() {
        JCLogUtil.d(TAG, "releaseAllVideos");
        JCVideoPlayerManager.completeAll();
        JCMediaManager.instance().releaseMediaPlayer();
    }
...
//            jcVideoPlayer.setAnimation(ra);
    ...
    public static void hideSupportActionBar(Context context) {
        ...
        if (TOOL_BAR_EXIST) {
            ...
            if (Build.VERSION.SDK_INT < 16) {
                ...
                ViewGroup decorView = (ViewGroup) (JCUtils.scanForActivity(context)).getWindow().getDecorView();
    ...
    public void setPlayerBackgroundColor(@ColorInt int color){
        textureViewContainer.setBackgroundColor(color);
    }
